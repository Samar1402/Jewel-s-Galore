...
const SignUp = () => {
  ...
  // --- CHANGE START: New States for Security Key & Eye Toggle ---
  ...
  // --- CHANGE START: Handler to allow ONLY 4 numbers ---
  const handleKeyChange = (e) => {
    const val = e.target.value;
    // Regex explained: ^ start, \d digits only, {0,4} max 4 chars, $ end
    if (/^\d{0,4}$/.test(val)) {
        setSecurityKey(val);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:8080/auth/signup", {
        ...
        email,
        password,
        securityKey // <--- CHANGE: Send key to backend
      });
    ...
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="bg-white border border-gray-200 rounded-2xl shadow-2xl p-10 w-[90%] md:w-[400px] text-center text-gray-700">
        ...
        <form className="flex flex-col gap-5" onSubmit={handleSubmit}>
          ...
          {/* --- CHANGE START: New Input Field with Eye Icon --- */}
          <div className="relative w-full">
            <input
              type={showKey ? "text" : "password"} // Toggle input type
              placeholder="4-Digit Security Pin"
              value={securityKey}
              onChange={handleKeyChange} // Using our custom handler
            ...
            {/* Eye Icon Button Positioned Absolutely */}
            <button
              type="button"
