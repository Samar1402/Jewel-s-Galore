...
const Header = () => {
  ...
  const { user, logout } = useAuth();   // <-- Auth context working
  const [cartCount, setCartCount] = useState(0);

  // 🛒 Correct LocalStorage Cart Count
  useEffect(() => {
    ...
    <nav className="flex flex-wrap justify-between items-center py-3 px-6 md:px-10 bg-gradient-to-r from-[#1a1a1a] via-[#2c2c2c] to-[#1a1a1a] shadow-md">
      ...
      <div className="hidden md:flex gap-10 lg:gap-20 items-center font-serif text-base lg:text-lg text-[#f5e6c5]">
        ...
        {user && (
          <Link
            to="/cart"
            className="relative hover:text-[#d4af37] transition flex items-center font-medium"
          >
      ...
      <div className="flex items-center gap-4 md:gap-6 mt-3 md:mt-0">
        ...
        <a
          ...
          className="text-[#25D366] text-2xl hover:text-[#d4af37] hover:scale-110 transition"
        >
          <FaWhatsapp />
        </a>
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-[#E1306C] text-2xl hover:text-[#d4af37] hover:scale-110 transition"
        >
          <FaInstagram />
        </a>
        ...
        {!user ? (
          <Link to="/login">
            <button className="w-28 md:w-32 text-[#1a1a1a] font-semibold text-base md:text-lg h-9 md:h-10 rounded-xl bg-gradient-to-r from-[#d4af37] to-[#f5e6c5] shadow-lg hover:scale-105 transition">
              Login
            </button>
          </Link>
        ) : (
          <button
            ...
            className="w-28 md:w-32 text-white font-semibold text-base md:text-lg h-9 md:h-10 rounded-xl bg-red-600 shadow-lg hover:bg-red-700 hover:scale-105 transition"
          >
            Logout
          </button>
        )}
